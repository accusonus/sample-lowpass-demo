...
class AudioUnitManager {
    ...
    public func updateProcessingAmount(value: Float) {
        let keyName = "processingAmount"
        let paramTree = self.audioUnit?.parameterTree
        let auParameter = paramTree?.value(forKey: keyName) as? AUParameter
        auParameter?.value = AUValue(value)
    }
    
    public func shouldBypass(value: Bool) {
        let keyName = "bypass"
        let paramTree = self.audioUnit?.parameterTree
        let auParameter = paramTree?.value(forKey: keyName) as? AUParameter
        auParameter?.value = AUValue(value ? 1 : 0)
    ...
    func loadAudioUnits(ofType type: AudioUnitType, completion: @escaping ([Component]) -> Void) {
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            let description = AudioComponentDescription(componentType: componentType,
                                                        componentSubType: 0,
                                                        componentManufacturer: 0x41434355,
                                                        componentFlags: 0,
