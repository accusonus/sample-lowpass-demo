...
class MainViewController: UIViewController {
    ...
    @IBOutlet weak var processingSlider: UISlider!
    ...
    override func viewDidLoad() {
        ...
        // MARK: - Accusonus specific code
        // Denoise
        var lowPassDescription = AudioComponentDescription()

        lowPassDescription.componentType = kAudioUnitType_Effect
        lowPassDescription.componentSubType = 0x6c777073 /*'lwps'*/
        lowPassDescription.componentManufacturer = 0x41434355 /*'ACCU'*/

        lowPassDescription.componentFlags = 0
        lowPassDescription.componentFlagsMask = 0

        AUAudioUnit.registerSubclass(LowPassAudioUnit.self, as: lowPassDescription, name: "ACCU:LowPass", version: 1000)


        // MARK: - Accusonus specific code End
    ...
    @IBAction func buttonChanged(_ sender: UISwitch) {
        audioUnitManager.shouldBypass(value: !sender.isOn)
    }
    
    
    @IBAction func sliderChanged(_ sender: UISlider) {
        audioUnitManager.updateProcessingAmount(value: sender.value)
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case audioUnitsTableView:
            audioUnitManager.selectComponent(at: indexPath.row) { result in
                switch result {
                case .success:
                    self.processingSlider.setValue(0.5, animated: true)
